...
function chipEstado(item: EvidenciaPendiente, sincronizando: boolean) {
  if (item.estado === 'sincronizado') return { label: 'Sincronizado', className: 'bg-lime-500/15 text-lime-txt' }
...
export default function EvidenciasPage() {
  ...
  const pendientes = items.filter((i) => i.estado !== 'sincronizado').length
    ...
    <AppShell title="Fotos y videos" subtitle="Evidencia del trabajo" back={{ label: 'Tablero', to: '/ordenes-trabajo' }}>
      ...
      <section className="mt-3.5 rounded-2xl bg-app-surface p-4" style={{ border: '1px solid var(--color-app-line)' }}>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {items.map((item) => {
            const chip = chipEstado(item, sincronizando)
            return (
              <div key={item.uuid} className="flex items-center gap-3">
                <div className="flex size-[52px] shrink-0 items-center justify-center rounded-[11px] bg-app-surface-3">
                  {item.tipoArchivo.startsWith('video') ? <Video size={22} className="text-app-faint" /> : <Image size={22} className="text-app-faint" />}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[13.5px] font-medium">{item.nombreArchivo}</p>
                  <p className="text-[11.5px] text-app-faint">
                    {tamano(item.archivo.size)} · {hace(item.tomadaAt)}
                  </p>
                </div>
                <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-medium ${chip.className}`}>{chip.label}</span>
              </div>
            )
          })}
          {items.length === 0 && <p className="py-4 text-center text-sm text-app-muted">Todavía no agregaste fotos o videos.</p>}
        </div>
